...
const FullscreenPlayer = ({
  ...
  useEffect(() => {
    async function fetchMovies() {
      ...
      const { data, error } = await supabase
        .from(
          `content?select=*,user_watch_history(*,movie_id)&user_watch_history.user_id=eq.${user?.id}`,
        )
        .select("*")
  ...
  return (
    <ProtectedRoute>
      <div
        className="fixed inset-0 z-[9999] overflow-y-hidden overflow-x-hidden"
        style={{
        ...
        <div className="w-full mx-auto p-2 sm:p-8 h-full overflow-y-auto">
          <div className="max-w-7xl mx-auto px-0 sm:px-4 py-6 sm:py-12">
            <div className="flex justify-center items-center gap-4 mb-4">
              <div
                className="flex items-center justify-start gap-4 cursor-pointer flex-1"
                onClick={async () => {
                  console.log(
                    "🔙 [FullscreenPlayer] Back button clicked, saving and refetching...",
                  );
                  // Ensure we save progress to the DB before fetching fresh data
                  if (
                    parentRef.current &&
                    (parentRef.current as any).saveProgress
                  ) {
            ...
            <div className="flex items-center mx-auto justify-center aspect-[4/3] sm:aspect-video w-full bg-black rounded-lg sm:rounded-2xl overflow-hidden mb-8 relative shadow-2xl border border-white/10">
              ...
              <div
                ref={playerRef}
                className="flex items-center justify-center h-full w-full sm:min-w-full md:sm:min-w-full"
              >
                {actualVideoUrl &&
                  (playlistId || contentIds ? (
                    <PlaylistVideoPlayer
                      ...
                      movieId={
                        isSeries
                          ? seriesContentId || movieId
                          : selectedContent?.id || movieId
                      }
                      episodeId={
                        isSeries ? selectedContent?.id || movieId : undefined
                      }
                      userid={user?.id}
                      ...
                      completionThreshold={
                        selectedContent?.completion_threshold_seconds ||
                        currentEpisode?.completion_threshold_seconds ||
                        currentMovie?.completion_threshold_seconds
                      }
                    />
                  ) : (
                    <VideoPlayer
                      ...
                      completionThreshold={
                        ...
                        currentEpisode?.completion_threshold_seconds ||
                        currentMovie?.completion_threshold_seconds ||
                        selectedContent?.content?.completion_threshold_seconds
                      }
                    />
                  ))}
              </div>
